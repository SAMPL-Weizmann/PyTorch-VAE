...
class WAE_MMD_REP(BaseVAE):
    ...
    def __init__(self,
                 ...
                 recon_factor: int = 10,
                 **kwargs) -> None:
        super(WAE_MMD_REP, self).__init__()
    ...
    def forward(self, input: Tensor, **kwargs) -> List[Tensor]:
        z = self.encode(input)
        blond = z[:, 0]
        return [self.decode(z), input, z, blond]
    ...
    def loss_function(self,
        ...
        recons_loss = self.recon_factor * F.mse_loss(recons, input)

        mmd_loss = self.recon_factor * self.compute_mmd(z, reg_weight)

        loss = recons_loss + mmd_loss + blond_hair_loss
        return {'loss': loss, 'Reconstruction_Loss':recons_loss, 'MMD': mmd_loss, 'blond_hair_loss': blond_hair_loss}
